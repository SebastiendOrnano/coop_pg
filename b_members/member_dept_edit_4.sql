...
SELECT 
    ...
    'select'                AS type,
    (SELECT member_publish FROM members  WHERE member_id = $member_id::INTEGER ) AS value,
    3                     AS width,
...
SELECT 
   ...
   'Saisissez quelques lettres du titre que vous cherchez ou choississez dans la liste' AS description,
    (SELECT orga_dept_id FROM members WHERE member_id = $member_id::INTEGER ) AS value,
     json_agg(JSON_BUILD_OBJECT('label',o.orga_name||'('||o.orga_id||')--'||d.orga_dept_name||'('||d.orga_dept_id ||')', 'value', d.orga_dept_id))  AS options
    FROM orga_dept AS d
    LEFT JOIN orga AS o 
