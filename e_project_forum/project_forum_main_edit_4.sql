...
SET project_forum_message_id = SELECT project_forum_message_id FROM project_forum_messages  WHERE project_forum_topic_id=$project_forum_topic_id::INTEGER AND project_forum_message_rank='main';
...
SELECT 
    ...
    (SELECT project_doc_cloud_url 
    FROM project_docs 
    WHERE project_forum_message_id=$project_forum_message_id::INTEGER
    AND project_doc_cloud_url IS NOT NULL    
    ) AS value,
