...
SELECT 
 ...
 SELECT 
    ...
    2 AS width,
    TRUE                     as required,
    (SELECT prm3_status  FROM prm3 WHERE prm3_id = $prm3_id::INTEGER ) as value,
...
SELECT
    ...
    'textarea' as type,  
    'Pitch' as label,
    (SELECT prm3_summary  FROM prm3 WHERE prm3_id = $prm3_id::INTEGER ) as value;
...
SELECT 
    ...
    'select'                       as type,
    3                              AS width,
    (SELECT prm3_type  FROM prm3 WHERE prm3_id = $prm3_id::INTEGER ) as value,
    json_agg(JSON_BUILD_OBJECT('label',i.choice_label, 'value', i.choice_value))  AS options
    FROM choices_items AS i
    LEFT JOIN (SELECT choice_category_id, choice_category_name FROM choices_categories)  AS c 
    ON i.choice_category_id = c.choice_category_id
...
SELECT 
    ...
    'select'                       as type,
    3                              AS width,
    (SELECT prm3_category  FROM prm3 WHERE prm3_id = $prm3_id::INTEGER ) as value,
...
SELECT 
    ...
    'select'                       as type,
    3                              AS width,
      (SELECT prm3_formal  FROM prm3 WHERE prm3_id = $prm3_id::INTEGER ) as value,
    'Choisir un statut dans la liste...'    as  empty_option,
...
SELECT 
    ...
    'date'                      as type,
    (SELECT prm3_date_creation  FROM prm3 WHERE prm3_id = $prm3_id::INTEGER ) as value,
    3                            AS width,
