...
SELECT 
   ...
   'Clear' AS reset,
    '/b_orga_dept_partners/orga_partner_update_0.sql?orga_partner_id='||$orga_partner_id AS action;

SELECT 
    'orga_dept_id' AS name,
    'Rappel nom du département ou de la filiale de rattachement' AS label,
    TRUE          AS readonly,
...
SELECT
    ...
    TRUE AS required,
    8 AS width,
    (SELECT orga_partner_name  FROM orga_dept_partners WHERE orga_partner_id = $orga_partner_id::INTEGER ) AS value;
...
SELECT
    ...
    'Sigle' AS label,
    4 AS width,
    (SELECT orga_partner_acronym  FROM orga_dept_partners WHERE orga_partner_id = $orga_partner_id::INTEGER ) AS value;

SELECT 
    'orga_partner_punchline' AS name,
    'text' AS type,
    'Punch line' AS label,
    (SELECT orga_partner_punchline  FROM orga_dept_partners WHERE orga_partner_id = $orga_partner_id::INTEGER ) AS value;
...
SELECT
    ...
    4 AS width,
    (SELECT orga_partner_scope  FROM orga_dept_partners WHERE orga_partner_id = $orga_partner_id::INTEGER ) AS value;
 ...
 SELECT 
    ...
    'select'                       AS type,
    2                              AS width,
    (SELECT orga_partner_type  FROM orga_dept_partners WHERE orga_partner_id = $orga_partner_id::INTEGER ) AS value,
 ...
 SELECT 
    ...
    'select'                       AS type,
    2                               AS width,
    (SELECT orga_partner_category  FROM orga_dept_partners WHERE orga_partner_id = $orga_partner_id::INTEGER ) AS value,
 ...
 SELECT 
    ...
    'select'                     AS type,
    2                            AS width,
     (SELECT orga_partner_status  FROM orga_dept_partners WHERE orga_partner_id = $orga_partner_id::INTEGER ) AS value,
    json_agg(JSON_BUILD_OBJECT('label',i.choice_label, 'value', i.choice_value))  AS options
    FROM choices_items AS i
    LEFT JOIN (SELECT choice_category_id, choice_category_name FROM choices_categories)  AS c 
    ON i.choice_category_id = c.choice_category_id
 ...
 SELECT 
    ...
    'date'                       AS type, 
     (SELECT orga_partner_date_since  FROM orga_dept_partners WHERE orga_partner_id = $orga_partner_id::INTEGER ) AS value,
    2                            AS width,
...
SELECT
    ...
    'Lien vers le site du partenaire' AS label,
     (SELECT orga_partner_link  FROM orga_dept_partners WHERE orga_partner_id = $orga_partner_id::INTEGER ) AS value,
...
SELECT 
    ...
    (SELECT orga_partner_summary FROM orga_dept_partners WHERE orga_partner_id = $orga_partner_id::INTEGER ) AS value;
