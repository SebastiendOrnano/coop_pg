...
SELECT
    ...
    12                      AS width,
    TRUE                   AS required,
    (SELECT news_title FROM news WHERE news_id = $news_id::INTEGER ) AS value;
 ...
 SELECT 
    ...
    3                            AS width,
    TRUE                          AS required,
    (SELECT news_public_private FROM news WHERE news_id = $news_id::INTEGER ) AS value,
 ...
 SELECT 
    ...
    3                            AS width,
    TRUE                         AS required,
    (SELECT news_status  FROM news WHERE news_id = $news_id::INTEGER ) AS value,
    'Choisir un statut dans la liste...' AS  empty_option,
...
SELECT 
    ...
    'Date'                      AS label,
    'date'                      AS type,
    (SELECT news_date  FROM news WHERE news_id = $news_id::INTEGER ) AS value,
    3                            AS width,
...
SELECT 
    ...
    'select'                       AS type,
    3                              AS width,
    (SELECT news_category  FROM news WHERE news_id = $news_id::INTEGER ) AS value,
...
SELECT
    ...
    (SELECT news_content  FROM news WHERE news_id = $news_id::INTEGER ) AS value,
    TRUE                           AS required;
