...
SELECT 'table' AS component, 
    ...
    'Alert'          AS markdown,
    JSON('{"name":"View","tooltip":"Voir le détail de la réunion","link":"/e_project_meeting/project_meeting_main_view_4.sql?project_meeting_id={id}","icon":"user-circle"}') as custom_actions,
    JSON('{"name":"Participantsl","Liste des participants":"Voir le profil","link":"/e_project_meeting/project_meeting_participants_display_4.sql?project_meeting_id={id}","icon":"users"}') as custom_actions,
    '/e_project_meeting/project_meeting_main_delete_0.sql?project_meeting_id={id}' as delete_url,
    TRUE             AS sort, 
    TRUE             AS small,
    TRUE             AS search;
...
THEN JSON('{"name":"Edit","tooltip":"Voir le détail de la réunion","link":"/e_project_meeting/project_meeting_main_edit_4.sql?project_meeting_id='||e.project_meeting_id||'","icon":"edit"}')
ELSE  JSON('{"name":"Edit"}')
