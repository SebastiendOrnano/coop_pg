...
SELECT 
    '/e_project_meeting/project_meeting_participant_form1_4.sql?project_meeting_id='||$project_meeting_id       AS link,
   'Inscrire une personne physique (PRM1)'       AS description,
    'copy-plus'                                  AS icon,
    'green'                                      AS color;

SELECT 
    '/e_project_meeting/project_meeting_participant_form2_step1_4.sql?project_meeting_id='||$project_meeting_id       AS link,
...
SELECT 'table' AS component, 
   ...
   JSON('{"name":"Remove","tooltip":"Retirer de la liste des participant","link":"/e_project_meeting/project_meeting_participant_remove_0.sql?project_meeting_participant_id={id}","icon":"logout"}') as custom_actions,
    JSON('{"name":"Role","tooltip":"Changer je rôle du participant","link":"/e_project_meeting/project_meeting_participant_edit_4.sql?project_meeting_participant_id={id}","icon":"switch"}') as custom_actions;
SELECT
    p.project_meeting_participant_id                    AS Id,
    p.project_meeting_participant_id                    AS _sqlpage_id,
