...
SELECT 'table' AS component, 
    'Forum forum_topics'   AS title, 
     JSON('{"name":"OPEN","tooltip":"Rouvrir la discussion","link":"/c_workspaces/workchat_open_0.sql?topic_id={id}","icon":"lock-open-2"}') as custom_actions,
    TRUE             AS sort, 
    TRUE             AS search;
...
WHERE t.topic_status ='archived' AND t.forum_type='chat_wksp' AND t.workspace_id=$workspace_id::INTEGER AND (m.message_author_id = $user_id::INTEGER OR m.message_recipient_id = $user_id::INTEGER)
