...
SET doc_link = 
CASE
WHEN (SELECT workdoc_url FROM workdocs WHERE message_id=$message_id_main::INTEGER) <>'' 
THEN  concat('http://localhost:8080/', (SELECT workdoc_url FROM workdocs WHERE message_id=$message_id_main::INTEGER ))
ELSE ''
END;

SET doc_title = 
CASE WHEN (SELECT workdoc_url FROM workdocs WHERE message_id=$message_id_main::INTEGER) <>'' 
...
SELECT 'form'      AS component,
 ...
 '/c_workspaces/workforum_message_create_0.sql?topic_id='||$topic_id AS action;
