SELECT
    ...
    format('attachment; filename="geo_group_id_%s.geojson"', $geo_group_id) AS "Content-Disposition";
...
SELECT
  ...
  jsonb_build_object(
    ...
    'features', jsonb_agg(ST_AsGeoJSON(g.*)::jsonb)
  )::text AS contents
...
WHERE p.geo_group_id = $geo_group_id::integer AND g.geo_selected = TRUE;
/*
