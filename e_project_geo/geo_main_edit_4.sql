...
SELECT 
    'form' AS component,
    'Actualiser la description de l''élément topo' AS title,
...
SELECT
    ...
    12 AS width,
    TRUE AS required,
    (SELECT geo_title  FROM project_geo WHERE geo_id=$geo_id::INTEGER ) AS value;
...
SELECT
    ...
    2                        AS width,
    (SELECT geo_code_postal  FROM project_geo WHERE geo_id=$geo_id::INTEGER ) AS value;
...
SELECT
    ...
    'Code postal' AS label,
    2                        AS width,
    (SELECT geo_code_insee  FROM project_geo WHERE geo_id=$geo_id::INTEGER ) AS value;
...
SELECT
    ...
    2                       AS width,
    (SELECT geo_code_land_registry  FROM project_geo WHERE geo_id=$geo_id::INTEGER ) AS value;
...
SELECT 
   ...
   'Bâtiment en lien avec la parcelle' AS label,
    ...
    LEFT JOIN (SELECT geo_group_id, project_id FROM project_geo_group) AS g 
    ON m.project_id = g.project_id
    LEFT JOIN (SELECT geo_group_id, building_id, geo_id from project_geo) AS p
    ON g.geo_group_id= p.geo_group_id
    WHERE p.geo_id=$geo_id::INTEGER ;
...
SELECT
    ...
    (SELECT geo_description  FROM project_geo WHERE geo_id=$geo_id::INTEGER ) AS value;
