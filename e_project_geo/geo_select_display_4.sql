SELECT 'dynamic' AS component, sqlpage.run_sql('a_shells/shell_4.sql') AS properties;

SET project_id = SELECT project_id FROM project_geo_group WHERE geo_group_id=$geo_group_id::INTEGER ;

SELECT 
    'breadcrumb' AS component;
SELECT 
    'Home' AS title,
    '/'    AS link;
SELECT 
    'Gestion superviseur'               AS title,
    '/a_panels/panel_supervisor_4.sql' AS link;
 SELECT 
    'Projets'                                   AS title,
    '/e_project/project_main_display_4.sql'     AS link;   
SELECT 
    'Gestion des groupes topo'                                                AS title,
    '/e_project_geo/geo_group_display_4.sql?project_id='||$project_id     AS link;
SELECT 
    'Gestion des éléments topo'                                                AS title,
    '/e_project_geo/geo_main_display_4.sql?geo_group_id='||$geo_group_id     AS link;


SELECT 
    'datagrid'              AS component,
    'panel_project_geo_doc_display' AS id,
    'Traitement sur une selection d''éléments topo' AS title;
SELECT 
    '/e_project_geo/geo_select_all_0.sql?geo_group_id='||$geo_group_id      AS link,
    'Tout sélectionner'                                      AS description,
    'list-check'                                     AS icon,
    'red'                                               AS color;
SELECT 
    '/e_project_geo/geo_select_no_0.sql?geo_group_id='||$geo_group_id     AS link,
    'Annuler toute la sélection'               AS description,
    'stack'                                    AS icon,
    'red'                                   AS color;
SELECT 
    '/e_project_geo/geo_select_edit_4.sql?geo_group_id='||$geo_group_id     AS link,
    'Actualiser des données sur les éléments sélectionnés'       AS description,
    'refresh'                                  AS icon,
    'blue'                                   AS color;
SELECT 
    '/e_project_geo/geo_select_delete_alert_0.sql?geo_group_id='||$geo_group_id      AS link,
    'Supprimer les éléments sélectionnés'       AS description,
    'books'                                    AS icon,
    'yellow'                                   AS color;
SELECT 
     '/e_project_geo/geo_select_geojson_export_0.sql?geo_group_id='||$geo_group_id            AS link,
    'Exporter les éléments sélectionnés'                                         AS description,
    'file-download'                                                                           AS icon,
    'yellow'                                                                               AS color;



SELECT 
    'table'                 AS component, 
    'Liste des documents'   AS title, 
    'View'                  AS markdown,
    'Titre'                  AS markdown,
    'download_selection_table' AS id,
    TRUE                   AS sort, 
    TRUE                   AS search;

SELECT  
    geo_id        AS Id,
    geo_title    AS Titre,
    geo_status    AS statut,
    geo_selected  AS selected,
  '[select / unselect](geo_select_yes_0.sql?geo_id='||geo_id||')'    AS View
FROM project_geo
WHERE geo_group_id = $geo_group_id::INTEGER
ORDER BY created_at DESC;




