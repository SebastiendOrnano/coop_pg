...
SELECT 
    'map'   AS component,
    'Lo calisation de l''élément' AS title,
    16 AS zoom,
    150 AS height,
...
SELECT 
    'form' AS component,
    '' AS validate,
    'Description de l''élément' AS title;
...
SELECT
 ...
 SELECT 
    ...
    TRUE AS readonly,
    2                        AS width,
    (SELECT  ROUND(ST_Area(geo_geom::geography))::integer FROM project_geo WHERE geo_id=$geo_id::INTEGER ) AS value;
...
SELECT 
   'building_id' AS name,
    ...
    WHERE l.geo_id=$geo_id::INTEGER ) AS value;
