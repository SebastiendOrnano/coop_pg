...
SELECT 
    '/e_project_geo/geo_coordo_form_3.sql?project_id='||$project_id     AS link,
    'Création d''un tracé'                                                 AS description,
    'route'                                                                AS icon,
    'green'                                                                AS color;
SELECT 
    '/e_project_geo/geo_geojson_single_upload_3.sql?project_id='||$project_id     AS link,
    'Charger un fichier geojson simple'                                              AS description,
    'upload'                                                                         AS icon,
    'blue'                                                                           AS color;
SELECT 
    '/e_project_geo/geo_geojson_collection_upload_3.sql?project_id='||$project_id     AS link,
    'Charger un fichier geojson complexe'                                                AS description,
    'upload'                                                                             AS icon,
    'red'                                                                                AS color;
SELECT 
     '/e_project_geo/geo_map_display_all_3.sql?project_id='||$project_id             AS link,
    'Visualiser toutes les parcelles et points d''un projet'                            AS description,
    'map-2'                                                                             AS icon,
    'green'                                                                             AS color;
SELECT 
     '/e_project_geo/geo_map_building_form_3.sql?project_id='||$project_id             AS link,
    'Visualiser toutes les parcelles et points d''un bâtiment'                        AS description,
    'map-2'                                                                           AS icon,
    'red'                                                                             AS color;
SELECT 
     '/e_project_geo/geo_select_display_3.sql?project_id='||$project_id             AS link,
