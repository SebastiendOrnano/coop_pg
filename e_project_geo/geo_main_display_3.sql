...
SELECT 
    '/e_project_geo/geo_coordo_form_4.sql?geo_group_id='||$geo_group_id     AS link,
    'Création d''un tracé'                                                 AS description,
    'route'                                                                AS icon,
    'green'                                                                AS color;
SELECT 
    '/e_project_geo/geo_geojson_single_upload_4.sql?geo_group_id='||$geo_group_id     AS link,
    'Charger un fichier geojson simple'                                              AS description,
    'upload'                                                                         AS icon,
    'blue'                                                                           AS color;
SELECT 
    '/e_project_geo/geo_geojson_collection_upload_4.sql?geo_group_id='||$geo_group_id     AS link,
    'Charger un fichier geojson complexe'                                                AS description,
    'upload'                                                                             AS icon,
    'red'                                                                                AS color;
SELECT 
     '/e_project_geo/geo_group_map_4.sql?geo_group_id='||$geo_group_id             AS link,
    'Visualiser toutes les éléments du groupe topo'                            AS description,
    'map-2'                                                                             AS icon,
    'green'                                                                             AS color;
SELECT 
     '/e_project_geo/geo_map_building_form_4.sql?geo_group_id='||$geo_group_id             AS link,
    'Visualiser toutes les parcelles et points d''un bâtiment'                        AS description,
    'map-2'                                                                           AS icon,
    'red'                                                                             AS color;
SELECT 
     '/e_project_geo/geo_archive_display_4.sql?geo_group_id='||$geo_group_id             AS link,
...
SELECT 
    ...
    'green'                                                                               AS color;
SELECT 
     '/e_project_geo/geo_geojson_export_collection_group_0.sql?geo_group_id='||$geo_group_id             AS link,
    'Exporter toutes les éléments du groupe topo'                                      AS description,
...
SELECT 'table' AS component, 
    ...
    JSON('{"name":"Map","tooltip":"Localiser la parcelle ou le point","link":"/e_project_geo/geo_map_display_3.sql?geo_id={id}","icon":"map-pin-check"}') AS custom_actions,
    JSON('{"name":"Data","tooltip":"Données","link":"/e_project_geo/geo_main_profile_3.sql?geo_id={id}","icon":"file-spreadsheet"}') AS custom_actions,
