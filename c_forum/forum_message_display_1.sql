...
SELECT
    'Contenu du message initial' AS name,
    'textarea'         AS type,
    true               as readonly,
    ...
    THEN 
          json_build_array(
                json_build_object(
                    ...
                    'link', '/c_forum/forum_message_edit_1.sql?message_id='||m.message_id,
                    'icon', 'edit'
                ),
                 json_build_object(
                    'name', 'Delete',
                    'tooltip', 'Supprimer le message',
                    'link', '/c_forum/forum_message_delete_0.sql?message_id='||m.message_id,
                    'icon', 'trash'
                )
            )
        ELSE
