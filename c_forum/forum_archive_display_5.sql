...
SELECT 'table' AS component, 
    ...
    '/c_forum/forum_main_edit_5.sql?topic_id={id}' as edit_url,
    '/c_forum/forum_main_delete_alert1_5.sql?topic_id={id}' as delete_url,
    JSON('{"name":"topic","tooltip":"Voir la discussion","link":"/c_forum/forum_message_display_5.sql?topic_id={id}","icon":"transform"}') as custom_actions,
    JSON('{"name":"close","tooltip":"Clore la discussion","link":"/c_forum/forum_main_close_0.sql?topic_id={id}","icon":"lock"}') as custom_actions,
    TRUE                   AS sort, 
    TRUE                   AS search;
...
WHERE  t.topic_status ='archived' AND t.forum_type='forum' AND t.topic_status != 'active'
